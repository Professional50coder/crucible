...
> - **Win32 end-to-end fine-tune landed — Passport #3** (part of B): a fresh fine-tune (task `d06d00e2…`) was retrieved, acknowledged and minted on native Windows via `HttpModelRetriever`, with a real on-chain-verified adapter root `0x113b79c3…8a396c` and `verifyManifest(3, 0x2e38e49c…)` = **true**. mint tx `0x1dde66f4…66fff3` (block 55,457,526), ack tx `0xaf0a48b0…01290b1`. `runs/run4-e2e.json`. *(Driven by the `tools/run4-*` scripts calling the real retriever + validation; carrying it through `POST /jobs` and the daemon on win32 is the part of B still to do.)*
> - **Attestation earned (D)**: `verifyService` passes (signer + compose match, `runs/attestation-testnet.json`), so Passport #3 was minted with `tee.attestationVerified: true` — the flag stands for those two checks, not full TDX quote validation. #1 and #2 keep `false`.
> - **Still open:** A (mainnet deploy + mint), the C/E items below, the orchestrator-daemon half of B on Windows, and a `retrieval.ts` transport with streaming/resume for artifacts over ~60 MB.
...
> **Landed 2026-09-18 (win32 fine-tune half done).** A complete fine-tune was retrieved, acknowledged and minted on native Windows as **Passport #3** — task `d06d00e2…`, `Init → … → Delivered → UserAcknowledged`, 93,642,471-byte adapter re-derived to on-chain root `0x113b79c3…8a396c` before ack, `verifyManifest(3)` = true, `attestationVerified: true`. mint `0x1dde66f4…66fff3` / ack `0xaf0a48b0…01290b1`. Evidence `runs/run4-e2e.json`, `runs/run4/mint.json`. It used the real `HttpModelRetriever` + validation but was driven by `tools/run4-*` scripts, not `POST /jobs`; the orchestrator-daemon path on Windows and the upload-path move below remain to do.
...
#### D. Attestation earned — landed 2026-09-18 (on Passport #3)
> **Done for a stated meaning.** `verifyService` passes on the provider (`runs/attestation-testnet.json`, `success: true`): signer `0x24135b4B…5583A` matches on-chain, compose `8779f38c…` matches event log. **Passport #3 was minted with `tee.attestationVerified: true`** — the first passport to carry it. The flag stands for exactly those two checks; #1 and #2 keep `false` in their immutable manifests.
- `tee.attestationVerified` is no longer `false` everywhere — it is `true` on #3, `false` on #1/#2. `tools/verify-attestation.mjs` passes 2/3 SDK-named steps `docs/FIELD_NOTES.md:144`. **Still to do for the fuller meaning:** run the `dstack-verifier:0.5.4` container for TDX quote + RTMR replay + OS hash, so the flag can stand for full quote validation rather than signer + compose alone.
